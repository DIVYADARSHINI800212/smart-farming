...
import { Microscope, AlertTriangle, MapPin, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export const DiseaseResultCard: React.FC<DiseaseResultCardProps> = ({ disease }) => {
  return (
    <Card className="border-red-200 shadow-card">
      <CardHeader
        title="Edge Disease Classification Diagnostic"
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="p-4 rounded-xl bg-red-50/50 border border-danger-red/30">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Badge severity={disease.severity}>{disease.severity} SEVERITY</Badge>
              ...
              <span className="text-xs font-bold text-danger-red bg-red-100 px-2.5 py-0.5 rounded-full">
                Affected Area: {disease.affectedAreaPercentage}% (0.41 Acres)
              </span>
            </div>
            ...
            <div className="mt-3">
              ...
              <p className="text-xs text-gray-500 italic mt-0.5">
                Causal Organism: {disease.scientificName}
              </p>
            </div>
          ...
          <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-subtle space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400">
              Multi-Class Prediction Probability Output
            </h4>
        ...
        <div className="space-y-4 flex flex-col justify-between">
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
            <h4 className="text-xs font-bold uppercase tracking-wider text-deep-green mb-2.5">
              Verified Symptom Manifestations
            </h4>
          ...
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-yellow-800">
            <strong className="block font-bold">Phenology Vulnerability:</strong>
            {disease.progressionStage}. Early control prevents disease progression to panicle neck blast.
          </div>
        </div>
      </div>
    </Card>
