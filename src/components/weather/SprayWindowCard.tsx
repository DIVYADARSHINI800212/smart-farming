  ...
  const getStatusBadge = (status: SprayWindowSuitability['overallStatus']) => {
    switch (status) {
      case 'GOOD APPLICATION WINDOW':
        return <Badge variant="success">OPTIMAL APPLICATION WINDOW</Badge>;
      case 'MARGINAL WINDOW':
        return <Badge variant="warning">MARGINAL WINDOW</Badge>;
      case 'NO-SPRAY WINDOW':
      default:
        return <Badge variant="danger">NO-SPRAY CONDITIONS</Badge>;
    }
  };
    ...
    <Card className="p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-semibold text-agri-dark">Pesticide & Foliar Spray Advisory</h3>
            <p className="text-xs text-agri-muted">Drift avoidance, droplet evaporation & rainfastness assessment</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-emerald-50/80 to-teal-50/60 border border-emerald-100 mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-agri-green">
              Recommended Spray Timing: {currentWindow.date}
            </span>
          </div>
      ...
      <div className="space-y-2">
        <h4 className="text-xs font-bold uppercase text-agri-muted tracking-wider">All Upcoming Evaluation Windows</h4>
            ...
            <div key={idx} className="p-3 rounded-lg border border-slate-100 bg-white text-xs space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                  ...
                  {w.overallStatus === 'GOOD APPLICATION WINDOW' ? 'Optimal' : w.overallStatus === 'MARGINAL WINDOW' ? 'Marginal' : 'No Spray'}
                </span>
              </div>
              <p className="text-[11px] text-agri-muted truncate">{w.date} • {w.tempValue}</p>
            </div>
          ))}
        </div>
      </div>
