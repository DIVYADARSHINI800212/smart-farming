...
export const SoilMoistureChart: React.FC<SoilMoistureChartProps> = ({ data }) => {
  ...
  return (
    <Card className="flex flex-col justify-between">
      <CardHeader
        title={t('chart_soil_moisture_title', 'Soil Moisture & Water Dynamics')}
        subtitle={t('chart_soil_moisture_sub', 'Zone 1 vs Zone 2 vs Optimal Agronomic Threshold')}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line
              ...
              name={`Zone 1 ${t('soil_moisture', 'Moisture')} (%)`}
              stroke="#F4B942"
              strokeWidth={2.5}
              dot={{ r: 3, fill: '#F4B942' }}
              activeDot={{ r: 5 }}
            ...
            <Line
              ...
              name={`Zone 2 ${t('soil_moisture', 'Moisture')} (%)`}
              stroke="#2E7D32"
              strokeWidth={2.5}
              dot={{ r: 3, fill: '#2E7D32' }}
              activeDot={{ r: 5 }}
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
        <span className="flex items-center gap-1.5">
          ...
          {t('optimal_range', 'Green Band = Optimal Root Capacity (40% - 60%)')}
        </span>
        <span className="text-amber-700 font-semibold">{t('water_deficit_z1', 'Zone 1 Requires Irrigation')}</span>
      </div>
    </Card>
  );
};
