...
import { Bug, MapPin, Gauge } from 'lucide-react';
...
export const PestResultCard: React.FC<PestResultCardProps> = ({ pest }) => {
  ...
  return (
    <Card className="border-amber-200 shadow-card">
      <CardHeader
        title={t('pest_detection_title', 'Edge Entomology Inference Diagnostic')}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="p-4 rounded-xl bg-amber-50/50 border border-warning-amber/40">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Badge severity={pest.severity}>{t(`badge_${pest.severity.toLowerCase()}`, pest.severity)} {t('severity', 'SEVERITY')}</Badge>
              ...
              <span className="text-xs font-bold text-yellow-900 bg-amber-100 px-2.5 py-0.5 rounded-full">
                {t('trap_status', 'Trap Count')}: {pest.trapCountPerAcre} / {t('acres', 'acre')}
              </span>
            </div>

            <div className="mt-3">
              <h3 className="text-2xl font-black text-deep-green tracking-tight">
                {t('detected_pest_name', pest.pestName)}
              </h3>
              <p className="text-xs text-gray-500 italic mt-0.5">
                {t('pathogen_type', 'Scientific Taxon')}: {pest.scientificName}
              </p>
            </div>
          ...
          <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-subtle space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400">
              {t('class_probability_dist', 'Entomological Classification Probabilities')}
            </h4>
        ...
        <div className="space-y-4 flex flex-col justify-between">
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-3">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-deep-green">
              <Gauge className="h-4 w-4 text-warning-amber" /> {t('damage_threshold', 'Economic Threshold Status')}
            </div>

            <div className="p-3 bg-white rounded-lg border border-amber-200">
              <span className="text-[11px] text-gray-400 block font-semibold">{t('pest_density', 'Infestation Level')}</span>
            ...
            <div className="p-3 bg-white rounded-lg border border-gray-200 text-xs">
              <span className="text-[11px] text-gray-400 block font-semibold">{t('damage_threshold', 'ETL Guideline')}</span>
          ...
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-yellow-800">
            <strong className="block font-bold">{t('actionable_guidance', 'Field Advisory')}:</strong>
            {t('pest_threshold_exceeded', 'Damaged leaves at 14% (above 10% tillering ETL). Initiate biological parasitoid release.')}
          </div>
        </div>
      </div>
    </Card>
